...
----------------------------------------------------------------- */
			...
			window.onload = function () {
				...
				// 소켓 생성	
                // 소켓 생성 (socket.io -> WebSocket 래퍼)
		var socket = (function () {
			// Java 서버 주소 (포트는 Anygrow2Server와 동일해야 함)
			var ws = new WebSocket("ws://localhost:52273");
			...
			ws.onmessage = function (event) {
				var msg = event.data;          // 예: "serial_recive:0,2,0,0,...,ff,ff"
				var idx = msg.indexOf(":");
				if (idx < 0) return;

				var eventName = msg.substring(0, idx);   // "serial_recive"
				var payload   = msg.substring(idx + 1);  // "0,2,0,0,...,ff,ff"
			...
			return {
				// socket.emit('serial_write', "On") 처럼 사용 가능
				emit: function (eventName, payload) {
					var msg = eventName + ":" + payload;
				...
				// socket.on('serial_recive', function (data) { ... }) 처럼 사용 가능
